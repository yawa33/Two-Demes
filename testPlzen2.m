...
(*ParallelDo[Clear[pp];
 pp = {Nest[
    IterateDemes5[#, {4000, 4000}, {.012, .0125}, {.12, .12}, {1/2, 
       1/2}, {2, 2}, .03, .5] &, {pop, uop}, 20]};
 Do[pp = ReplacePart[
    pp, -1 -> {Insert[pp[[-1]][[1]], 0, 
       Position[pp[[-1]][[1]], 0, 3, 1]], 
      Insert[pp[[-1]][[2]], 1, Position[pp[[-1]][[2]], 1, 3, 1]]}];
  
  pp = Join[
    pp, {Nest[
      IterateDemes5[#, {4000, 
         4000}, {.012, .0125}, {.12, .12}, {1/2, 1/2}, {2, 
         2}, .03, .5] &, pp[[-1]], 25]}];
  , {110}];
 
 Flatten[{Transpose[
       Table[{Count[
           Flatten[pp[[i]][[#]], 1], {0}]/(2 Length[pp[[i]][[#]]]), 
         Count[Flatten[pp[[i]][[1]], #], {0, 
            0}]/(2 Length[pp[[i]][[#]]]), 
         Count[Flatten[
            pp[[i]][[1]], #], {1}]/(2 Length[pp[[i]][[#]]]), 
         Count[Flatten[pp[[i]][[1]], #], {1, 
            1}]/(2 Length[pp[[i]][[#]]]), 
         Count[Flatten[pp[[i]][[1]], #], {1, 
            0}]/(2 Length[pp[[i]][[#]]]), 
         Count[Flatten[
            pp[[i]][[1]], #], {_, _, _}]/(2 Length[
             pp[[i]][[#]]])}, {i, Length[pp]}]]} & /@ {1, 2}, 1] >>> 
  "/storage/plzen1/home/yawa/DEMES/TTripl2";
 Print[{"round done", Plus @@ Flatten[pp[[#]]] & /@ {1, 2}}]
 ,
 {8}]*)
...
{Count[Flatten[pp[[i]][[#]], 1], {0}]/(2 Length[pp[[i]][[#]]]), 
 ...
 {i, Length[pp]}]]} & /@ {1,
      2}, 1] >>> "/storage/plzen1/home/yawa/DEMES/testNP1";
 Print[{"round done", Plus @@ Flatten[pp[[#]]] & /@ {1, 2}}], {8}]
